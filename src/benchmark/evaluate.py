...
from src.engines import engine as engine_lib
...
def evaluate_single_puzzle(args: Tuple[pd.Series, str]) -> Dict:
    ...
    return {"puzzle_id": puzzle.name, "correct": correct, "rating": puzzle["Rating"]}


def evaluate_puzzle_from_pandas_row(
    puzzle: pd.Series, engine: engine_lib.Engine
) -> bool:
    """
...
def evaluate_puzzles(
    ...
    return {"results": results, "rating_intervals": dict(rating_intervals)}


def save_evaluation_results(
    results: Dict, agent_name: str, output_dir: str = "results"
):
    """
...
def plot_puzzle_results(results: Dict, out_path: str, puzzle_result_name: str):
    ...
    sorted_intervals = sorted(
        rating_intervals.items(), key=lambda x: int(x[0].split("-")[0])
    )
