...
def tokenize(fen: str,config:dict):
  transform = TRANSFORMATION_DICT[config['model_choice']]
  ...
  if transform == 'tensor':
      ...
      for piece in KQ_dict:
          if piece in castling_str:
              castling_plane[KQ_dict[piece][0],KQ_dict[piece][1]] = 1 # this should be okay, they are still at the original places
      ...
      out_tensor = np.concatenate([piece_array, castling_plane_3d, enpass_plane_3d], axis=2)  # (8,8,8)
      ...
      enpass_file_ohe = enpass_file_ohe.reshape(16,)
