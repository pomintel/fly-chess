...
from src.fly_unit import data_loader as data_loader
from src.fly_unit.train import train
from src.fly_unit.evaluate import eval_DPU_on_puzzles
  ...
  for base_config['timesteps'] in [2,5,10,15]:
      for exp_id in experiments.keys():
          ...
          if 'filter_num' in droso_config:
              droso_config['exp_id'] = exp_id + f"_{droso_config['filter_num']}filters"+ f"_{str(num_records)}"
          else:
              droso_config['exp_id'] = exp_id + f"_{str(num_records)}"
