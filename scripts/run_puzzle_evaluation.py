...
def main():
    ...
    parser.add_argument(
        "--load_existing",
        action="store_true",
        help="Load existing results instead of evaluating",
    )
    ...
    if args.agent not in constants.ENGINE_BUILDERS:
        raise ValueError(
            f"Unknown agent: {args.agent}. Available agents: {list(constants.ENGINE_BUILDERS.keys())}"
        )
    ...
    print(
        f"Workers used: {args.num_workers if args.num_workers else max(1, os.cpu_count() - 1)}"
    )
    print(
        f"Results saved to: {os.path.join(args.output_dir, f'{args.agent}_rating_histogram.png')}"
    )
